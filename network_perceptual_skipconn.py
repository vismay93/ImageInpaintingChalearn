...
class Network:
    def __init__(self, x, y, mask, is_training, batch_size):
        ...
        self.y = x*(1-tf.round(self.mask)) + y*tf.round(self.mask)    
        ...
        self.perceptual_loss = self.calc_perceptual_loss(self.completion, self.y)
        self.real = self.discriminator(self.y, reuse=False)
        self.fake = self.discriminator(self.completion, reuse=True)

        self.g_loss = self.calc_g_loss(self.y, self.imitation, self.mask)
        self.d_loss = self.calc_d_loss(self.real, self.fake)
        self.gan_loss = self.calc_gan_loss(self.y,self.imitation, self.mask,self.fake)
        
        self.total_g_loss = tf.add(1e-12*self.perceptual_loss,self.g_loss)
        self.total_gan_loss = tf.add(1e-12*self.perceptual_loss,self.gan_loss)
    ...
    def calc_d_loss(self, real, fake):
        ...
        return tf.add(d_loss_real,6* d_loss_fake)

    def calc_perceptual_loss(self, completion, gt):
        data_dict = loadWeightsData('/media/data/anubha/inpainting/train/src/vgg16.npy')
