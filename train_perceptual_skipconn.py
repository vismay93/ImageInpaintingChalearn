...
PRETRAIN_PATH = './model/'
CHECKPOINT_PATH = './model/20latest'
DATASET_PATH= './Data'
...
def train():
	with tf.device('/device:GPU:0'):
	    ...
	    mask = tf.placeholder(tf.float32, [BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE])
	    is_training = tf.placeholder(tf.bool, [])

	    model = Network(x, y, mask,is_training, batch_size=BATCH_SIZE)
	    ...
	    dl = dataLoader(DATASET_PATH,'train',BATCH_SIZE,IMAGE_SIZE)
	    N = dl.nofiles()
	    ...
	    while True:
	        ...
	        if sess.run(epoch) <= PRETRAIN_EPOCH:
	            ...
	            for i in tqdm.tqdm(range(step_num)):
	                ...
	                """
	                if(i%500 == 0):
	                    imi,comp,y_b = sess.run([model.imitation,model.completion,model.y],feed_dict={x: x_batch, y: y_batch, mask: m_batch, is_training: True})
	                    #save_images(PRETRAIN_PATH+'train',sess.run(epoch),i,y_b,comp,imi,x_batch)
	                """
